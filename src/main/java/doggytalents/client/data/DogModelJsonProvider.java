...
public class DogModelJsonProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput output) {
        ...
        ClientSetup.setupEntityRenderers(new RegisterLayerDefinitions());
        for (var entry : ClientSetup.LAYER_DEFS.entrySet()) {
            var id = entry.getKey();
            if (!id.equals(ClientSetup.DOG))
                continue;
            var model = entry.getValue();
            var encoded_model = DTNModelCodec.parsedFromLayerDefintion(model.get());
            var anim_json_optional = DTNModelCodec.CODEC.encodeStart(JsonOps.INSTANCE, encoded_model).result();
            if (!anim_json_optional.isPresent())
                continue;
            var anim_json = anim_json_optional.get();
            var save_path = path_prov.json(id.getModel().withPath(x -> x + "_" + id.getLayer()));
            var save_future = 
                saveStableNoArrayIndent(output, anim_json, save_path);
