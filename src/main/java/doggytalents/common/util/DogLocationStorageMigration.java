...
public class DogLocationStorageMigration {
    ...
    public static void checkAndMigrate(ServerLevel level) {
        ...
        try {
            newStorage = savedData.get(DogLocationStorage.savedDataType_1_21_5(Constants.STORAGE_DOG_LOCATION));
        } catch (Exception e) {}
        if (newStorage != null)
            return;
        ...
        try {
            oldStorage = savedData.get(DogLocationStorage.savedDataType_1_21_5(Constants.STORAGE_DOG_LOCATION_OLD));
        } catch (Exception e) {}
        if (oldStorage == null)
            return;
