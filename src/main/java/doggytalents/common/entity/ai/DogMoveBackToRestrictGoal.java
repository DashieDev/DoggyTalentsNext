...
public class DogMoveBackToRestrictGoal extends MoveTowardsRestrictionGoal {
    ...
    private boolean teleportBackIfNeeded() {
        if (!dog.hasDogRestriction())
            return false;
        var center = dog.getDogRestrictCenter();
        if (center == null)
            return false;
        ...
        if (!dog.level().hasChunkAt(center))
            return false;
        return DogUtil.guessAndTryToTeleportToBlockPos(dog, center, Mth.floor(dog.getDogRestrictRadius()));
    }
}
