...
public class DogTextureServer {
    ...
    public InputStream getCachedStream(File baseFolder, String hash) {
        ...
        if (cacheFile.isFile() && cacheFile.exists()) {
            try {
                ...
                DoggyTalentsNext.LOGGER.debug("Loaded dog texture from local cache ({})", cacheFile);
                return stream;
            } catch (FileNotFoundException e) {
                DoggyTalentsNext.LOGGER.debug("Failed to load dog texture from local cache ({})", cacheFile);
                e.printStackTrace();
            }
        }
    ...
    public boolean saveTexture(File baseFolder, byte[] stream) throws IOException {
        ...
        DoggyTalentsNext.LOGGER.debug("Server already has cache for dog texture ({})", cacheFile);
        return false;
    }
}
